const PersonalModel = require('../../models/user/personalmodels');
...
const getPersonalDetails = (req, res) => {
    PersonalModel.getAll((err, results) => {
        ...
        results = results.map(person => ({
            ...person,
            profileImage: person.profileImage ? `http://localhost:8000${person.profileImage}` : null,
            coverImage: person.coverImage ? `http://localhost:8000${person.coverImage}` : null
        }));
