const AdvisorModel = require('../../models/user/advisorModel');
...
const getAdvisors = (req, res) => {
    AdvisorModel.getAll((err, results) => {
        ...
        results = results.map(advisor => ({
            ...advisor,
            profile_image: advisor.profile_image ? `http://localhost:8000${advisor.profile_image}` : null,
            cover_image: advisor.cover_image ? `http://localhost:8000${advisor.cover_image}` : null
        }));
